...
async def create_product(
    product: ShoppingProductCreate, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(
        select(ShoppingProduct).where(ShoppingProduct.name == product.name)
    )
...
async def add_to_list(item: ShoppingListItemCreate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(ShoppingProduct).where(ShoppingProduct.id == item.product_id)
    )
